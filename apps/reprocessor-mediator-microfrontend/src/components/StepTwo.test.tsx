...
describe('SummaryScreen Component', () => {
    ...
    test('renders component with initial state', () => {
        ...
        expect(screen.getByText(`${data.reprocessFromDate} - ${data.reprocessToDate}`)).toBeInTheDocument()
        expect(screen.getByText(/Number of Transactions/i)).toBeInTheDocument()
        expect(screen.getByText(`${data.numberOfTransactions}`)).toBeInTheDocument()
        expect(screen.getByText(/Resources to be Reprocessed/i)).toBeInTheDocument()
    ...
    test('clicking the "Reprocess" button triggers the correct API call', async () => {
        ...
        render(<SummaryScreen data={data} onBack={handleBack} onCancel={handleCancel} />)
    ...
    test('handles failed reprocess request', async () => {
        global.fetch = jest.fn().mockRejectedValue(new Error('Failed to perform the Reprocess Request'))
        const handleBack = jest.fn()
        const handleCancel = jest.fn()
        ...
        await waitFor(() => {
            expect(consoleErrorSpy).toHaveBeenCalledWith('Error encountered on reprocess POST Request:', 'Failed to perform the Reprocess Request');
        });
